    ...
    def test(self):
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(
                f"Saved model not found: {self.model_path}\n"
                "Train first without --test_only (same --model, --dataset, -tag/--model_name_suffix, "
                "--seed, --export_dir) so train() writes this .pth. "
                "--test_only skips training, so no weights file is created."
            )
