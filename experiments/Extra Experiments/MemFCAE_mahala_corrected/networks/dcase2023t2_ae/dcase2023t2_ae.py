...
class DCASE2023T2AE(BaseModel):
    ...
    def init_model(self):
        ...
        return AENet(
            ...
            use_memcae=getattr(self.args, 'use_memcae', True),
            num_memory_slots=getattr(self.args, 'num_memory_slots', 64),
            memory_sparsity_weight=getattr(self.args, 'memory_sparsity_weight', 0.0)
        )
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            if not is_calc_cov:
                # For Memory-Augmented FC AE, add weighted attention entropy sparsity loss
                if getattr(self.args, 'use_memcae', True) and getattr(self.args, 'memory_sparsity_weight', 0.0) > 0.0:
                    ...
                    self.loss = recon_loss + self.args.memory_sparsity_weight * sparsity_loss
                self.loss.backward()
                self.optimizer.step()
