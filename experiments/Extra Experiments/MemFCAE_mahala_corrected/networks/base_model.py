...
class BaseModel(object):
    def __init__(self, args, train, test):
        ...
        print("\n" + "="*60)
        print("Running MemFCAE with Corrected Mahalanobis")
        print("Source folder copied from experiments/MemFCAE")
        print(f"Dataset: {self.args.dataset}")
        print(f"Model weights path loaded: {self.model_path}")
        stats_path = self.model_dir/f"score_distr_{self.args.model}_{self.args.dataset}{self.model_name_suffix}{self.eval_suffix}_seed{self.args.seed}_mahala.pickle"
        print(f"Covariance/statistics path loaded: [{self.model_path}] and [{stats_path}]")
        print("Corrected Mahalanobis: enabled")
        active_result_dir = self.eval_data_result_dir if getattr(self.args, 'eval', False) else self.result_dir
        print(f"Result directory: {active_result_dir}")
        print("="*60 + "\n")
