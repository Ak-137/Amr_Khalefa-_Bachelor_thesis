...
from networks.dcase2023t2_ae.network import AENet, LATENT_DIM
from networks.criterion.mahala import loss_function_mahala, calc_inv_cov
from networks.criterion.mahala_latent import (
    accumulate_latent_cov,
    calc_inv_cov_latent,
    mahalanobis_per_sample,
)
from tools.plot_anm_score import AnmScoreFigData
from tools.plot_loss_curve import csv_to_figdata

# This experiment: encoder latent z (dim=LATENT_DIM) for Σ fit + Mahalanobis scores.
# Training still uses MSE from baseline.yaml; do NOT gate latent covariance on --score MAHALA
# (that flag is False during train, which previously forced the 128×128 residual path and crashed).
USE_LATENT_MAHALANOBIS = True
...
class DCASE2023T2AE(BaseModel):
    ...
    def init_model(self):
        self.block_size = self.data.height  # n_mels; used for MSE reshaping only
        self.latent_dim = LATENT_DIM  # encoder bottleneck; Mahalanobis cov is (8, 8)
        return AENet(input_dim=self.data.input_dim, block_size=self.block_size)

    def _use_latent_space_mahala(self):
        """True for this experiment: Σ and distances live in z-space (not residual mel-blocks)."""
        return USE_LATENT_MAHALANOBIS

    def _latent_rows(self, z, mask_list):
        """Select batch rows of z (N, latent_dim) using boolean mask per sample."""
        idx = torch.tensor(mask_list, device=z.device, dtype=torch.bool)
        return z[idx]
    ...
    def train(self, epoch):
        ...
        if epoch == self.args.epochs + 1:
            ...
            # Fit Sigma on normal training latents z (per source/target), not on residuals.
            cov_x_source = torch.zeros((self.latent_dim, self.latent_dim), device=self.device)
            cov_x_target = torch.zeros((self.latent_dim, self.latent_dim), device=self.device)
            num_source = 0
            num_target = 0
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            if is_calc_cov:
                # Encoder latent z: shape (batch_size, latent_dim), one vector per mel-frame patch.
                if self._use_latent_space_mahala():
                    if n_source > 0:
                        z_source = self._latent_rows(z, is_source_list)
                        cov_x_source = accumulate_latent_cov(z_source, cov_x_source)
                    num_source += n_source
                    if n_target > 0:
                        z_target = self._latent_rows(z, is_target_list)
                        cov_x_target = accumulate_latent_cov(z_target, cov_x_target)
                        num_target += n_target
                    score_2d = F.mse_loss(recon_batch, data.view(recon_batch.shape), reduction="none").mean(dim=1)
                else:
                    ...
                    from networks.criterion.mahala import cov_v
            ...
            # loss reduction
            n_loss = len(score_2d)
            if is_calc_cov and self._use_latent_space_mahala():
                # Latent cov pass: score_2d is (batch,) from mean over feature dim; no dim=1 reduction.
                score = score_2d
            else:
        ...
        if is_calc_cov:
            ...
            if self._use_latent_space_mahala():
                inv_cov_source, inv_cov_target = calc_inv_cov_latent(
                    model=self.model,
                    device=self.device
                )
            else:
    ...
    def calc_valid_mahala_score(self, data, y_pred, inv_cov_source, inv_cov_target):
        ...
        _, z = self.model(data)
        if self._use_latent_space_mahala():
            # Per-vector d_i, then mean over vectors in the batch (clip aggregation).
            d_source = mahalanobis_per_sample(z, inv_cov_source)
            d_target = mahalanobis_per_sample(z, inv_cov_target)
            loss_source = torch.mean(d_source)
            loss_target = torch.mean(d_target)
        else:
    ...
    def eval(
        ...
        for j, batch in enumerate(test_loader):
            ...
            if self.args.score == "MAHALA":
                if self._use_latent_space_mahala():
                    # Latent score: independent d_i per frame-embedding, mean -> file score.
                    d_source = mahalanobis_per_sample(z, inv_cov_source)
                    d_target = mahalanobis_per_sample(z, inv_cov_target)
                    loss_source = torch.mean(d_source)
                    loss_target = torch.mean(d_target)
