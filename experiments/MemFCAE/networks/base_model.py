...
class BaseModel(object):
    def __init__(self, args, train, test):
        ...
        # Safety Check 1: Enforce ToyRCCar dataset selection
        if self.args.dataset != "DCASE2025T2ToyRCCar":
            print("\nWARNING: Dataset name is not DCASE2025T2ToyRCCar! Stopping execution for safety.")
            raise ValueError(f"Safety Check Failure: This experiment is strictly configured for ToyRCCar dataset (DCASE2025T2ToyRCCar), but got '{self.args.dataset}'.")

        # Safety Check 2: Check if memory module is disabled
        if not getattr(self.args, 'use_memcae', True):
            print("\nWARNING: Memory module is disabled (use_memcae is False)!")

        # Safety Check 3: Check if running in ablation mode (sparsity weight = 0.0)
        if getattr(self.args, 'use_memcae', True) and getattr(self.args, 'memory_sparsity_weight', 0.0) == 0.0:
            print("\nNOTE: Memory sparsity weight is set to 0.0. Running in ABLATION mode without entropy sparsity regularization.")

        # Print Memory-Augmented FC Autoencoder ToyRCCar experiment startup logs
        print("\n" + "="*60)
        print("Running Memory-Augmented FC Autoencoder ToyRCCar experiment")
        print("Source folder: experiments/MemFCAE (copied from baseline_original)")
        print(f"Dataset Name: {self.args.dataset}")
        print(f"Dataset Directory: {self.args.dataset_directory}")
        print(f"Model Class Used: {self.model.__class__.__name__}")
        print(f"Memory Enabled: {getattr(self.args, 'use_memcae', True)}")
        if hasattr(self.model, 'memory_module') and getattr(self.args, 'use_memcae', True):
            print(f"Memory Slots: {self.model.memory_module.num_slots}")
            print(f"Latent Dimension: {self.model.memory_module.dim}")
            print(f"Memory Sparsity Weight: {getattr(self.args, 'memory_sparsity_weight', 0.0)}")
        else:
            print("Memory Slots: N/A")
            print("Latent Dimension: 8")
            print("Memory Sparsity Weight: N/A")
        active_result_dir = self.eval_data_result_dir if getattr(self.args, 'eval', False) else self.result_dir
        print(f"Output Result Directory: {active_result_dir}")
        print("="*60 + "\n")
