...
class BaseModel(object):
    def __init__(self, args, train, test):
        ...
        # Safety check: dataset must be DCASE2025T2ToyRCCar
        if self.args.dataset != "DCASE2025T2ToyRCCar":
            raise ValueError(f"Dataset must be DCASE2025T2ToyRCCar, but got: {self.args.dataset}")

        # Startup diagnostic printouts
        print("="*60)
        print("Running CORAL_Res ToyRCCar experiment")
        print("Source copied from baseline_original")
        print(f"Dataset: {self.args.dataset}")
        print("AE architecture: unchanged baseline FC AE")
        
        use_coral = getattr(self.args, "use_coral_res", False)
        coral_status = "enabled" if use_coral else "disabled"
        print(f"CORAL residual alignment: {coral_status}")
        print(f"CORAL mode: {getattr(self.args, 'coral_mode', 'None')}")
        print(f"CORAL eps: {getattr(self.args, 'coral_eps', 1e-5)}")
        print("Corrected Mahalanobis: enabled")
        print(f"Result directory: {self.args.result_directory}")
        print(f"Weight/checkpoint directory: {self.checkpoint_dir}")
        
        # Warnings and confirmations
        if test and self.args.score != "MAHALA":
            print("WARNING: Score is not MAHALA during test. Corrected Mahalanobis is recommended.")
        if not use_coral:
            print("WARNING: use_coral_res is False.")
        print("CONFIRMATION: No anomaly or test labels are used for fitting CORAL statistics.")
        print("="*60)
        ...
        if self.args.restart or getattr(self.args, 'cov_only', False):
