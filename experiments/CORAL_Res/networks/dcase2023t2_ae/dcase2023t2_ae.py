...
from networks.criterion.mahala import cov_v, loss_function_mahala, calc_inv_cov, cov_v_diff
...
class DCASE2023T2AE(BaseModel):
    ...
    def train(self, epoch):
        if not getattr(self.args, 'cov_only', False) and epoch <= self.epoch:
            return
        ...
        if epoch == self.args.epochs + 1 or getattr(self.args, 'cov_only', False):
            print("\n============== CALCULATE COVARIANCE ==============")
            is_calc_cov = True
            self.model.eval()
            torch.set_grad_enabled(False)
            
            # --- CORAL STATISTICS FITTING (Phase 1) ---
            if getattr(self.args, 'use_coral_res', True):
                print("\n================== FIT CORAL RESIDUAL STATISTICS ==================")
                res_source_list = []
                res_target_list = []
                for batch_idx, batch in enumerate(tqdm(train_loader, desc="Collecting residuals for CORAL")):
                    ...
                    diff = data.view(recon_batch.shape) - recon_batch
                    
                    if any(is_source_list):
                        res_source_list.append(diff[is_source_list].view(-1, self.block_size))
                    if any(is_target_list):
                        res_target_list.append(diff[is_target_list].view(-1, self.block_size))
                
                if len(res_source_list) > 0 and len(res_target_list) > 0:
                    R_s = torch.cat(res_source_list, dim=0)
                    R_t = torch.cat(res_target_list, dim=0)
                    self.model.fit_coral_statistics(R_s, R_t, eps=getattr(self.args, 'coral_eps', 1e-5))
                    print(f"CORAL fitted source/target residual counts: source={R_s.size(0)}, target={R_t.size(0)}")
                else:
                    print("WARNING: Could not fit CORAL because source or target residuals are empty!")
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            if is_calc_cov:
                diff = data.view(recon_batch.shape) - recon_batch
                diff_source_raw = diff[is_source_list].view(-1, self.block_size)
                diff_target_raw = diff[is_target_list].view(-1, self.block_size)
                
                if getattr(self.args, 'use_coral_res', True) and getattr(self.model, 'coral_fitted', 0) == 1:
                    diff_source_aligned = self.model.coral_align_res(diff_source_raw, domain="source")
                else:
                    diff_source_aligned = diff_source_raw
                    
                diff_target_aligned = diff_target_raw # target is unchanged
                
                if n_source > 0:
                    cov_diff_source, _ = cov_v_diff(diff_source_aligned)
                    cov_x_source_batch = cov_v(diff=cov_diff_source, num=1)
                    cov_x_source += cov_x_source_batch.clone().detach()
                    num_source += len(diff_source_aligned)
                    
                if n_target > 0:
                    cov_diff_target, _ = cov_v_diff(diff_target_aligned)
                    cov_x_target_batch = cov_v(diff=cov_diff_target, num=1)
                    cov_x_target += cov_x_target_batch.clone().detach()
                    num_target += len(diff_target_aligned)

                score_2d = torch.cat([diff_source_aligned**2, diff_target_aligned**2], dim=0)
                n_source = len(diff_source_aligned)
                n_target = len(diff_target_aligned)
                is_source_list = [True] * n_source + [False] * n_target
                is_target_list = [False] * n_source + [True] * n_target
            else:
        ...
        if is_calc_cov:
            ...
            print(f"covariance source/target shapes: source={cov_x_source.shape}, target={cov_x_target.shape}")
        ...
        print(f"saved model path: {self.model_path}")
        torch.save({
            'epoch':epoch,
    ...
    def calc_valid_mahala_score(self, data, y_pred, inv_cov_source, inv_cov_target):
        ...
        coral_align_fn_s = None
        if getattr(self.args, 'use_coral_res', True) and getattr(self.model, 'coral_fitted', 0) == 1:
            coral_align_fn_s = lambda d: self.model.coral_align_res(d, domain="source")
        ...
        loss_source, num = loss_function_mahala(
            ...
            use_precision=True,
            reduction=False,
            coral_align_fn=coral_align_fn_s
        )
        loss_source = self.loss_reduction(score=self.loss_reduction_1d(loss_source), n_loss=num)

        coral_align_fn_t = None
        if getattr(self.args, 'use_coral_res', True) and getattr(self.model, 'coral_fitted', 0) == 1:
            coral_mode = getattr(self.args, 'coral_mode', 'source_to_target')
            if coral_mode == 'source_to_target':
                coral_align_fn_t = lambda d: self.model.coral_align_res(d, domain="target")
            else:
                coral_align_fn_t = lambda d: self.model.coral_align_res(d, domain="pooled")
        ...
        loss_target, num = loss_function_mahala(
            ...
            use_precision=True,
            reduction=False,
            coral_align_fn=coral_align_fn_t
        )
    ...
    def eval(
        ...
        for j, batch in enumerate(test_loader):
            ...
            if self.args.score == "MAHALA":
                ...
                coral_align_fn_s = None
                if getattr(self.args, 'use_coral_res', True) and getattr(self.model, 'coral_fitted', 0) == 1:
                    coral_align_fn_s = lambda d: self.model.coral_align_res(d, domain=domain)
                ...
                if getattr(self.args, 'use_coral_res', True) and getattr(self.model, 'coral_fitted', 0) == 1:
                    coral_mode = getattr(self.args, 'coral_mode', 'source_to_target')
                    if coral_mode == 'source_to_target':
                        coral_align_fn_t = lambda d: self.model.coral_align_res(d, domain=domain)
                    else:
                        coral_align_fn_t = lambda d: self.model.coral_align_res(d, domain="pooled")
