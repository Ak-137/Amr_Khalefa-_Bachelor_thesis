...
class BaseModel(object):
    def __init__(self, args, train, test):
        self.args = args
        # Safety checks for Top-K
        if getattr(self.args, 'use_topk_score', False):
            ...
            if not self.args.test_only:
                raise ValueError("test_only must be True when using Top-K score aggregation.")
        ...
        # Explicit score distribution file paths for test loading (loaded from TopK_Baseline without ratio suffix)
        ...
        # Apply Top-K specific results and logs separation by ratio
        if getattr(self.args, 'use_topk_score', False):
            k_suffix = f"_k{int(round(self.args.topk_ratio * 100)):03d}"
            actual_export_dir = f"{self.export_dir}{k_suffix}"

            self.result_dir = Path(f"{args.result_directory}/dev_data/{actual_export_dir}_{args.score}/")
            self.result_dir.mkdir(parents=True, exist_ok=True)
            self.eval_data_result_dir = Path(f"{args.result_directory}/eval_data/{actual_export_dir}_{args.score}/")
            self.eval_data_result_dir.mkdir(parents=True, exist_ok=True)

            actual_base_name = f"{actual_export_dir}/{self.args.model}_{self.args.dataset}{self.model_name_suffix}{self.eval_suffix}_seed{self.args.seed}"
            self.logs_dir = Path(f"logs/{actual_base_name}")
            self.logs_dir.mkdir(parents=True, exist_ok=True)
            self.log_path = self.logs_dir / "log.csv"
