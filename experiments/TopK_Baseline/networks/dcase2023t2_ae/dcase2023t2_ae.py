...
class DCASE2023T2AE(BaseModel):
    ...
    def eval(
        ...
        for j, batch in enumerate(test_loader):
            ...
            if self.args.score == "MAHALA":
                raw_loss_source, num = loss_function_mahala(
                    recon_x=recon_data,
                    x=data,
                    block_size=self.block_size,
                    cov=inv_cov_source,
                ...
                raw_loss_target, num = loss_function_mahala(
                    recon_x=recon_data,
                    x=data,
                    block_size=self.block_size,
                    cov=inv_cov_target,
                ...
                frame_scores_source = self.loss_reduction_1d(raw_loss_source)
                frame_scores_target = self.loss_reduction_1d(raw_loss_target)
                
                if getattr(self.args, 'use_topk_score', True):
                    topk_ratio = getattr(self.args, 'topk_ratio', 0.1)
                    
                    # Source
                    n_source_frames = len(frame_scores_source)
                    k_source = int(np.ceil(topk_ratio * n_source_frames))
                    k_source = max(1, min(k_source, n_source_frames))
                    topk_source_vals, _ = torch.topk(frame_scores_source, k=k_source, largest=True)
                    loss_source_file = torch.mean(topk_source_vals).item()
                    
                    # Target
                    n_target_frames = len(frame_scores_target)
                    k_target = int(np.ceil(topk_ratio * n_target_frames))
                    k_target = max(1, min(k_target, n_target_frames))
                    topk_target_vals, _ = torch.topk(frame_scores_target, k=k_target, largest=True)
                    loss_target_file = torch.mean(topk_target_vals).item()
                    
                    final_score = min(loss_target_file, loss_source_file)
                    
                    if not hasattr(self, 'topk_print_count'):
                        self.topk_print_count = 0
                    if self.topk_print_count < 5:
                        orig_source_mean = torch.mean(frame_scores_source).item()
                        orig_target_mean = torch.mean(frame_scores_target).item()
                        orig_final_mean = min(orig_target_mean, orig_source_mean)
                        print(f"[DIAGNOSTIC] File {basename}:")
                        print(f"  - Top-K aggregation: ENABLED (topk_ratio={topk_ratio:.4f})")
                        print(f"  - Number of frame scores per file: {n_source_frames}")
                        print(f"  - K used: {k_source}")
                        print(f"  - Original Mean Score: {orig_final_mean:.6f}")
                        print(f"  - Top-K Score: {final_score:.6f}")
                        self.topk_print_count += 1
                else:
                    loss_source_file = self.loss_reduction(score=frame_scores_source, n_loss=num).item()
                    loss_target_file = self.loss_reduction(score=frame_scores_target, n_loss=num).item()
                    final_score = min(loss_target_file, loss_source_file)
                    ...
                    if self.topk_print_count < 5:
                        print(f"[DIAGNOSTIC] File {basename}:")
                        print(f"  - Top-K aggregation: DISABLED")
                        self.topk_print_count += 1
                        
                y_pred.append(final_score)
            else:
                # MSE path
                mse_frame_scores = self.loss_fn(recon_x=recon_data, x=data).mean(dim=1)
                n_frames = len(mse_frame_scores)
                if getattr(self.args, 'use_topk_score', True):
                    topk_ratio = getattr(self.args, 'topk_ratio', 0.1)
                    k = int(np.ceil(topk_ratio * n_frames))
                    k = max(1, min(k, n_frames))
                    topk_vals, _ = torch.topk(mse_frame_scores, k=k, largest=True)
                    final_score = torch.mean(topk_vals).item()
                    
                    if not hasattr(self, 'topk_print_count'):
                        self.topk_print_count = 0
                    if self.topk_print_count < 5:
                        orig_mean = torch.mean(mse_frame_scores).item()
                        print(f"[DIAGNOSTIC - MSE] File {basename}:")
                        print(f"  - Top-K aggregation: ENABLED (topk_ratio={topk_ratio:.4f})")
                        print(f"  - Number of frame scores per file: {n_frames}")
                        print(f"  - K used: {k}")
                        print(f"  - Original Mean Score: {orig_mean:.6f}")
                        print(f"  - Top-K Score: {final_score:.6f}")
                        self.topk_print_count += 1
                else:
                    final_score = mse_frame_scores.mean().item()
                    if not hasattr(self, 'topk_print_count'):
                        self.topk_print_count = 0
                    if self.topk_print_count < 5:
                        print(f"[DIAGNOSTIC - MSE] File {basename}:")
